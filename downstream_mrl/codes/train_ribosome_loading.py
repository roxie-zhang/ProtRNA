...
class RibosomeLoadingPredictionWrapper(pl.LightningModule):
    ...
    def forward(self, tokens, vector):
        x = vector
    ...
    def _common_step(self, batch, batch_idx, log_prefix: str):
        seq_encoded, vector, rl_target = batch
        preds = self(seq_encoded, vector)
...
def main(args):
    ...
    if args.init_params:
        model.load_state_dict(torch.load(args.init_params)['state_dict'])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to mrl data feature path"
    )
    parser.add_argument(
        "--lm_type", type=str,
        help="which pretrained language model is going to be used"
    )
    parser.add_argument(
        "--test_set", type=str,
