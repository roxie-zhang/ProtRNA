...
def validate(args, model, device, data_loader, criterion, threshold=None):
    model.eval()
    y_all = []
    p_all = []
    l_all = []
    with torch.no_grad():
        for batch_idx, (x0, y0) in enumerate(data_loader):
            x, y = x0.float().to(device), y0.to(device).float()
            #if y0.sum() ==0:
    ...
    met = metrics.MLMetrics(objective='binary', threshold=threshold)
