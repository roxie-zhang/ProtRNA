...
class MLMetrics(object):
    def __init__(self, objective='binary', threshold=None):
        self.objective = objective
        self.metrics = []
        self.threshold = threshold

    def update(self, label, pred, other_lst):
        met, _, threshold = calculate_metrics(label, pred, self.objective, self.threshold)
        self.threshold = threshold
        if len(other_lst)>0:
...
def roc(label, prediction):
    ...
    if ndim == 1:
        ...
        for i in range(num_labels):
            ...
            yodenJ_threshold = thresholds[(tpr - fpr).argmax()]
    return metric, curves, yodenJ_threshold
...
def calculate_metrics(label, prediction, objective, threshold):
    ...
    if (objective == "binary") | (objective == 'hinge'):
        ...
        auc_roc, roc_curves, calculated_threshold = roc(label, prediction)
        ...
        if not threshold:
            print("calculated best threshold:", calculated_threshold)
            threshold = calculated_threshold
        pred_class = prediction>threshold
