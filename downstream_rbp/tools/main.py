...
def configure_device():
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = device_args.device
    print("CUDA_VISIBLE_DEVICES set to:", os.environ["CUDA_VISIBLE_DEVICES"])

    return device_args, other_args

device_args, other_args = configure_device()
...
def main():
    ...
    if args.train:
        ...
        val_loader = torch.utils.data.DataLoader(SeqicSHAPE(data_path, is_val=True), \
            batch_size=args.batch_size*8, shuffle=False,  **kwargs)
        ...
        print("Train set:", len(train_loader.dataset))
        # print("Test  set:", len(test_loader.dataset))
        print("Val  set:", len(val_loader.dataset))
        ...
        for epoch in range(1, args.nepochs + 1):
            ...
            v_met, _, _ = validate(args, model, device, val_loader, criterion)
            ...
            if args.tfboard and writer is not None:
                ...
                writer.add_scalar('loss/val', v_met.other[0], epoch)
                writer.add_scalar('acc/val', v_met.acc, epoch)
                writer.add_scalar('AUC/val', v_met.auc, epoch)
            ...
            line='{} \t Val  Epoch: {}     avg.loss: {:.4f} Acc: {:.2f}%, AUC: {:.4f} ({:.4f})'.format(\
    ...
    if args.eval:
        ...
        met, y_all, p_all = validate(args, model, device, val_loader, criterion)
        print("> val {} auc: {:.4f} acc: {:.4f}".format(args.p_name, met.auc, met.acc))
        print("threshold selected based on Youden's J: {:.4f}".format(met.threshold))

        test_loader  = torch.utils.data.DataLoader(SeqicSHAPE(data_path, is_val=True), \
            batch_size=args.batch_size*8, shuffle=False, **kwargs)
        print("Test  set:", len(test_loader.dataset))

        met, y_all, p_all = validate(args, model, device, test_loader, criterion, threshold=met.threshold)
        print("> test {} auc: {:.4f} acc: {:.4f}".format(args.p_name, met.auc, met.acc, met.threshold))
        print("tp {} tn {} fp {} fn {} under threshold {:.4f}".format(met.tp, met.tn, met.fp, met.fn, met.threshold))
